  ...
  <div class="top">
    ...
    <div class="i_car">
      ...
      <div class="car_bg">
        ...
        <div class="price_a"><router-link  to="/Buycar">去购物车结算</router-link></div>
<!--        <router-link  to="/Buycar">去购物车结算</router-link>-->
        <!--End 购物车已登录 End-->
      </div>
    </div>
  </div>
...
<!--          “开业巨惠，北京专柜直供”，不光低价，“真”才靠谱！-->
        ...
        <div class="des_price">
          <span v-for="priced in price">
          本店价格：<b>{{priced.price}}</b><br/>
            </span>
          消费积分：<span>28R</span>
        </div>
        <div class="des_choice" v-for="( value,key) in goods">
          ...
          <label>
            ...
            <input class="checked" @click="check" type="radio" v-bind:id="val[0]"   v-bind:value="val[1]"  v-bind:name="key">{{val[0]}}
              </span>
          </label>
        </div>
        <div class="des_share">
...
<script>
    ...
    export default {
        ...
        methods:{
            check(){
                ...
                $.each(this.goods,function (key,val) {
                    ...
                    //当input标签里面内容单个被选中时提示undefined，只有所有标签的内容全部被选中时才会触发ajax
                ...
                if(this.isajax==1){
                    //当都被选中时取出标签中的内容并循环进一个新的数组
                    // this.id=''
                    $.each(this.goods,function (keys,value) {
                        _this.id = _this.id+"-"+$("input[name='"+keys+"']:checked").val()
                    })
                    axios.post('http://localhost/php/laravel/blog/public/api/show/product',{
                        id:_this.id,
            ...
            cat(){
                var _this=this
                var  attr_name=''
                $.each(this.goods,function (k,v) {
                    console.log($("input[name='"+k+"']:checked").attr("id"))
                    _this.names=$("input[name='"+k+"']:checked").attr("id"),
                        //获取name=k的input框中id的属性值
                        attr_name=attr_name+' '+k+":"+_this.names
                    //将属性与属性细分绑在一起用：隔开
                });
        ...
        mounted (){
             axios.post('http://localhost/php/laravel/blog/public/api/show/shop',{
                 goods_id:this.$route.query.goods_id
             })
                 .then(res=>{
                     console.log(res)
